...
### D18 — Automated positional-recall suite via vendored codeneedle
**Decision:** Added a 4th benchmark suite, `needle` (positional recall), by vendoring `github.com/mdsaad23/local_llm_benchmarking` (codeneedle) into `vendor/local_llm_benchmarking/` and shelling out to its `bench.py run` CLI — rather than reimplementing the logic or adding a git submodule. The existing `tools`/`niah`/`coding` suites are unchanged; codeneedle is purely additive. The benchmark UI now lets the user pick **which models** (not just suites) to run, executes them sequentially, and renders results in a per-suite sectioned table.
**Why:** codeneedle is the owner's own benchmark and is faithful as-is; running its CLI keeps it a swappable, isolated dependency that can be re-pulled to update. It speaks an OpenAI-compatible API, which Ollama exposes at `OLLAMA_BASE_URL + /v1` — the vendored client appends `/v1/chat/completions` itself, so no model-config TOMLs are needed; `--base-url`, `--model`, `--max-tokens`, and `--dump` CLI overrides are enough.
**Pros:** Fully automated, no manual CLI step. Model selection + sequential runs + stored per-model results + readable sectioned tables, all from one button. Live progress (overall model/suite pairs + per-model query progress parsed from the CLI's `[i/n]` lines) and a responsive Cancel that kills the in-flight subprocess while keeping results already collected. Backend stays decoupled from vendored internals (subprocess + JSON dump parsing in `src/evaluation/needle_eval.py`).
**Cons/Limitations:** Adds the vendored repo to the tree. Only the http_server corpus is shipped — the jquery corpus and its fixture were removed (D18a), so no JS corpora and no `esprima` dep. Context size is governed by how Ollama loaded the model, not by codeneedle.
**Impact:** New `needle_eval.py` (subprocess driver with progress callback + `request_cancel()`); `benchmark.py` route gains normalized per-suite result rows, per-model sub-progress + `cancelled` state, `GET /benchmark/models`, `GET /benchmark/summary`, and a typed `{suites, models}` start body. `start_ollama.ps1` now sets `OLLAMA_CONTEXT_LENGTH=32768` so the ~14K-token http_server prompt isn't truncated by the 4K default. `NEEDLE_*` settings added.
**What's lacking:** Per-query progress is only granular for the needle suite (parsed from CLI output); tools/niah/coding show an indeterminate per-model bar. Smoke-tested on llama3.2:3b (recall ~4.5%, as expected for the weakest model on a hard benchmark) — full matrix not yet run.
**Ideal state:** Per-query progress streamed into the suite table live for every suite; automatic per-model context sizing.
**Updated:** 2026-06-13 — D18a: removed the jquery corpus + fixture entirely (not selectable/runnable) per owner request; it needed 100K+ context, infeasible for 14B on 16GB. Dropped the now-unused `esprima` dependency. Added live per-model/overall progress bars and a safe Cancel button (kills the subprocess, preserves partial results).
